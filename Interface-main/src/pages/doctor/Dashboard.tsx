...
export default function DoctorDashboard() {
  ...
  const myVisits = useMemo(() => getVisitsByDoctor(doctorId), [getVisitsByDoctor, doctorId]);
  const myPrescriptions = useMemo(() => getPrescriptionsByDoctor(doctorId), [getPrescriptionsByDoctor, doctorId]);
  ...
  const severityStats = useMemo(() => {
    ...
    myVisits.forEach(visit => {
      const change = getSeverityChange(visit);
  ...
  const visitColumns = [
    ...
    },
    {
      ...
      header: 'Severity',
      render: (visit: typeof recentVisits[0]) => (
        <SeverityBadge change={getSeverityChange(visit)} score={visit.severity_score} />
      ),
    },
  ];
