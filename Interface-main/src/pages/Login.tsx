...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = login(userId, password, portal as 'admin' | 'doctor');
    
    if (result.success) {
      navigate(portal === 'admin' ? '/admin' : '/doctor');
    } else {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="card-healthcare p-8">
          <div className="text-center mb-8">
            <div className={`inline-flex p-4 rounded-2xl mb-4 ${isAdmin ? 'gradient-primary' : 'bg-gradient-to-br from-accent to-primary'}`}>
              {isAdmin ? (
          ...
          <div className="mt-6 pt-6 border-t border-border">
            <p className="text-xs text-center text-muted-foreground">
              {isAdmin ? (
                <>Demo: <code className="px-1 py-0.5 rounded bg-muted">admin_hospital</code> / <code className="px-1 py-0.5 rounded bg-muted">admin123</code></>
              ) : (
                <>Demo: <code className="px-1 py-0.5 rounded bg-muted">dr_cardiology</code> or <code className="px-1 py-0.5 rounded bg-muted">dr_neuro</code> / <code className="px-1 py-0.5 rounded bg-muted">doctor123</code></>
              )}
            </p>
          </div>
        </div>
