...
import { Visit, Patient } from "@/types/hospital";
...
export default function DoctorVisitTable({
  searchQuery,
  doctorName,
}: DoctorVisitTableProps) {
  ...
  const getSeverityTrend = (patientId: string) => {
    ...
    const diff =
      lastTwo[1].severity_score - lastTwo[0].severity_score;
            ...
            {filtered.map((v) => {
              ...
              return (
                <tr key={v.visit_id}>
                  ...
                  <td>
                    <span
                      className={`severity-badge severity-${v.severity_score}`}
                    >
                      {v.severity_score}/5
                    </span>
                  </td>
                  <td>
                    {trend === "increased" ? (
                      <span className="feedback-negative text-xs flex items-center gap-1">
                        <TrendingUp className="w-3 h-3" />
                        Worsening
                      </span>
                    ) : trend === "decreased" ? (
                      <span className="feedback-positive text-xs flex items-center gap-1">
                        <TrendingDown className="w-3 h-3" />
                        Improving
                      </span>
                    ) : (
                      <span className="feedback-neutral text-xs flex items-center gap-1">
